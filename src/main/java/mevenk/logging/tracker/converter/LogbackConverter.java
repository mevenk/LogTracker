...
import mevenk.logging.tracker.util.LogUtils;
...
public class LogbackConverter implements Converter {
	...
	public Log convert(Object obj) {
		...
		Log.Builder builder = Log.newBuilder().date(LogUtils.formatDate(le.getTimeStamp()))
				.level(le.getLevel().toString()).threadName(LogUtils.nullToEmpty(le.getThreadName()))
				.loggerName(LogUtils.nullToEmpty(le.getLoggerName()))
				.message(LogUtils.nullToEmpty(le.getFormattedMessage())).throwable(getThrowable(le)).mdc(getMdc(le));
	...
	private String getThrowable(ILoggingEvent le) {
		...
		while (throwableProxy != null) {
			throwable.append(throwableProxy.getClassName()).append(": ").append(throwableProxy.getMessage())
