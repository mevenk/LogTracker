...
public class Log4j2Converter implements Converter {
	...
	public Log convert(Object obj) {
		...
		Log.Builder builder = Log.newBuilder().date(LogUtils.formatDate(le.getTimeMillis()))
				.level(le.getLevel().toString()).threadName(LogUtils.nullToEmpty(le.getThreadName()))
				.loggerName(LogUtils.nullToEmpty(le.getLoggerName())).ndc(le.getContextStack().toString())
				.mdc(getMdc(le));
